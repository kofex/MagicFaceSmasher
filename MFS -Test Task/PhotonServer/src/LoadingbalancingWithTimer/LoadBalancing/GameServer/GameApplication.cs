...
namespace Photon.LoadBalancing.GameServer
{
    ...
    public class GameApplication : ApplicationBase
    {
        ...
        //Создаем список подключенных пиров к игровому серверу
        ...
        //-----------------------Операции со списком подключенных пиров к игровому серверу--------------------
        #region Операции со списком подключенных пиров к игровому серверу        
        public void AddToPeerList(PeerBase peer)
        {
            peers.Add(peer);
        }

        public void RemoveFromPeerList(PeerBase peer)
        {
            if (peer!=null && peers.Contains(peer))
                peers.Remove(peer);            
        }

        public List<PeerBase> GetPeerList()
        {
            return peers;
        }
        //-----------------------------------------------------------------------------------------------------
        ...
        protected virtual PeerBase CreateGamePeer(InitRequest initRequest)
        {
            ...
            //Добавляем подключенный пир к списку
            AddToPeerList(peer);
            return peer;
        }
        ...
        protected override void TearDown()
        {
            ...
            //Очищаем список при закрытии сервера
            peers.Clear();
