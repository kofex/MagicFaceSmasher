...
namespace Photon.LoadBalancing.GameServer
{
    ...
    public class OutgoingMasterServerPeer : OutboundS2SPeer
    {
        ...
        {
            //Обрабатываем эвент таймера, который приходит от Мастер Сервера            
            log.Info("MY:: onEvent evcode " + eventData.Code);
            log.Info("MY:: application peerCount " + application.PeerCount);
            
            foreach (PeerBase peer in application.GetPeerList())
            {
