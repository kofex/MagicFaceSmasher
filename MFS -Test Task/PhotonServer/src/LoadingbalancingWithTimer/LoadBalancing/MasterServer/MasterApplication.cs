...
namespace Photon.LoadBalancing.MasterServer
{
    ...
    public class MasterApplication : ApplicationBase
    {
        ...
        private int timeOut = 180000; // ms = 3 min
        private Timer timer;        
        ...
        protected virtual void Initialize()
        {
            ...
            //Создаем таймер для сброса статистики
            log.Info("MY:::Create timer");
            TimerCallback timerCb = new TimerCallback(OnTimer);
            timer = new Timer(timerCb, null, 0, timeOut);
            
        }

        //code 100 - сброс статистики
        private void OnTimer(object o)
        {            
            log.Info("MY:::OnTimer");
            EventData evData = new EventData(100, null);          
           
            //Если игроки в меню - они подключены к Мастер Сервверу, так что эвент идет все напрямую
            if (AppStats.PeerCountMaster > 0)
            {
                log.Info("MY:::PeerCount master > 0");
                AppStats.OnTimer(evData);
            }
            //Если игроки в игре - они на Игровом Сервере, поэтому перекидываем эвент через гейм сервер
            if (AppStats.PeerCountGameServer > 0)
            {
                log.Info("MY:::PeerCountGAmeServer > 0");
                foreach (KeyValuePair<string, IncomingGameServerPeer> pair in GameServers)
                {
                    log.Info("MY:::PAIR " + pair + " count " + pair.Value.PeerCount);
                    log.Info("MY:::PAIR conn state" + pair.Value.ConnectionState + " connected " + " connid " + pair.Value.ConnectionId);

                    pair.Value.SendEvent(evData, new SendParameters());
                }
            }
        }
